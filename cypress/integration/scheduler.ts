...
context('Scheduler tests', () => {
  ...
  it('User should not be able to see upcoming experiment times if there is none', () => {
    cy.login('user');
    cy.contains('Dashboard').click();
    cy.finishedLoading();

    cy.contains('Upcoming experiment times').should('not.exist');
    cy.logout();
  });

  it('User should not be able to see upcoming experiment times in DRAFT state', () => {
    const query = `
  ...
  it('Instrument scientist should not be able to see upcoming experiment times in DRAFT state', () => {
    cy.login('user');
    cy.changeActiveRole('Instrument Scientist');
    cy.finishedLoading();

    cy.finishedLoading();
    cy.contains('Upcoming experiment times').click();

    cy.contains('No records to display');
  });

  it('User should be able to see upcoming experiment times in BOOKED', () => {
    const query = `
    ...
    cy.contains('Upcoming experiment times').should('exist');
  ...
  it('Instrument scientist should be able to see upcoming experiment times in BOOKED', () => {
    cy.login('user');
    cy.changeActiveRole('Instrument Scientist');
  ...
  it('User should be able to see upcoming experiment times in CLOSED', () => {
    const query = `
  ...
  it('Instrument scientist should be able to see upcoming experiment times in CLOSED', () => {
    cy.login('user');
    cy.changeActiveRole('Instrument Scientist');
